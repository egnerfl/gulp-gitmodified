...
Git.prototype.getStatusByMatcher = function (matcher, baseDir, cb) {
  ...
  that.which(gitApp, function (err) {
    ...
    that.exec(gitApp, ['diff', 'HEAD', 'origin/master', '--name-status'], gitExtraOptions, function (err, stdout) {
      if (err) {
        return cb(new Error('Could not get git diff HEAD origin/master --name-status'));
      }
